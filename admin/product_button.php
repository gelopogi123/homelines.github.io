    ...
    <div class="modal fade" id="editprod_<?php echo $pid; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
				...
				<div class="container-fluid">
					<?php
						$a=mysqli_query($conn,"select * from product left join category on category.categoryid=product.categoryid left join supplier on supplier.supplierid=product.supplierid where productid='$pid'");
						$b=mysqli_fetch_array($a);
					?>
                    ...
                    <form role="form" method="POST" action="edit_product.php<?php echo '?id='.$pid; ?>" enctype="multipart/form-data">
						...
						<div class="form-group input-group">
                            ...
                            <select style="width:400px;" class="form-control" name="supplier">
								...
								<?php
									$s=mysqli_query($conn,"select * from supplier where supplierid != '".$b['supplierid']."'");
									while($srow=mysqli_fetch_array($s)){
										?>
											<option value="<?php echo $srow['supplierid']; ?>"><?php echo $srow['company_name']; ?></option>
										<?php
									}
								?>
							</select>
